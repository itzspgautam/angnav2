...
const UpdateEvent = () => {
  ...
  const { eventLoading, categories, error, success, selectedEvent } =
  ...
  const submitEventHandle = () => {
    ...
    Dispatch(updateEvent(poster, eventData, id));
  };

  useEffect(() => {
    if (!selectedEvent) return;
    setTitle(selectedEvent.title);
    setSpeaker(selectedEvent.guestSpeaker);
    setSpeakerDes(selectedEvent.guestSpeakerDes);
    setDes(selectedEvent.description);
    setVideoUrl(selectedEvent.videoUrl);
    setSelected(selectedEvent.categories);
    setContent(selectedEvent.content);
    setPosterPrev(selectedEvent.poster.url);
    setPosterPrevPublic(selectedEvent.poster.public_id);
