...
const AvatarUpdate = (props) => {
  ...
  const { avatarUpload, success } = useSelector((state) => state.User);
  ...
  const cropAndUpload = useCallback(async () => {
    try {
      ...
      Dispatch(updateAvatar(croppedImage));
    } catch (e) {
      console.error(e);
    }
              ...
              <>
                ...
                <Box>
                  <Progress
                    colorScheme="green"
                    size="lg"
                    value={avatarUpload ? avatarUpload.percent : 0}
                  />
                </Box>
              </>
            ) : (
          ...
          <ModalFooter>
            {image ? (
              <>
                ...
                <Button
                  isLoading={
                    avatarUpload && avatarUpload.uploading ? true : false
                  }
                  size="sm"
                  variant="solid"
                  colorScheme={"yellow"}
